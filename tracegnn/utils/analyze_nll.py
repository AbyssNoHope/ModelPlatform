...
def analyze_anomaly_nll(nll_list: np.ndarray,
    ...
    # prepare for analyze
    result_dict = {}
    # label_list is the test dataset really label
    ...
    def best_fscore_for_label(label):
        ...
        return F(nll_list[mask], label_list[mask] != 0, ab)


    best_fscore_total, thold = F(nll_list, is_anomaly_list, 'all')
    best_fscore_drop, _ = best_fscore_for_label(1)
    best_fscore_latency, _ = best_fscore_for_label(2)
    result_dict.update({
        'best_fscore': float(best_fscore_latency)
    })
