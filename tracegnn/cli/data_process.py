...
@click.option('--max-node-count', type=int, required=True, default=128)
@click.option('--max-depth', type=int, required=True, default=32)
def make_train_test(input_dir, output_dir, train_size, val_size, test_size,
    ...
    # open input db, inputdir=/service/origin/
    ...
    for g in tqdm(db, desc=f'Load from {input_dir}'):
        ...
        # g_key is the key of each graph, if the key some traces of some traces is same, in graph_bins,
        # they have the same key, and graph_bins(values) if the list of them  
        if g_key not in graph_bins:
            graph_bins[g_key] = []
        graph_bins[g_key].append(g)

    print('len(graph_bins):', len(graph_bins))
    ...
    def do_resample(graph_bins, output_path, population_size):
        ...
        print('bin_sample_size:', bin_sample_size)
        trace_graphs = []
        ...
        for bin_array in tqdm(graph_bins.values(), desc=desc):
            print('len(bin_array)', len(bin_array))
