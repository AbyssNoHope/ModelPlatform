...
class ExpConfig(mltk.Config):
    ...
    class dataset(mltk.Config):
        root_dir: str = os.path.abspath('./data/service2/processed')
...
def main(exp: mltk.Experiment[ExpConfig]):
    ...
    # load newest model to train
    if config.train.continue_learn or config.train.incremental_train:
        print('continue learning!')
        save_model_path = 'results/train/models/final.pt'
        # load old params
        ...
        old_params = []
        for p in params:
            old_params.append(p.clone().detach())  
    ...
    def train_step(trace_graphs):
        ...
        # compute fisher
        def estimate_fisher():
            F_accum = [0.0 for v in range(len(params))]
            for i in range(len(params)):
                F_accum[i] += torch.sum(params[i].grad**2)/config.train.batch_size
                            
        # EWC regularization
        if config.train.incremental_train:
            # F_accum = estimate_fisher()
            for v in range(len(params)):
                loss = loss + 0.5*config.train.ewc_lambda*torch.sum((old_params[v]-params[v])**2)/config.train.batch_size
    ...
    def evaluate(n_z, eval_loop, eval_stream, epoch, use_embeddings=False,
                 plot_latency_hist=False, save_filename=None, std_max_limit = None):
        # latency_hist_file
    ...
    def save_model(epoch=None):
        epoch = epoch or loop.epoch
        # torch.save(vae,exp.make_parent(f'models/{epoch}.pt'))
    ...
    loop.run_after_every(save_model, epochs=10)
    ...
    if config.train.limit_std:
        std_result_dict = validate_std()
